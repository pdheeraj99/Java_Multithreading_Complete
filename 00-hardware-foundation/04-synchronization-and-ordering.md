...
*   This is a critical performance optimization, but it can break naive concurrent code. A classic example is where Thread 1 sets a value and then a flag, but a reordering causes the flag to be set *before* the value, leading Thread 2 to read a stale, uninitialized value.
...
*   The CPU writes to this buffer at high speed and immediately continues executing other instructions. The buffer then drains its contents to the L1 cache and eventually to main memory in the background.
...
---
### Sequential Consistency vs. The Java Memory Model
