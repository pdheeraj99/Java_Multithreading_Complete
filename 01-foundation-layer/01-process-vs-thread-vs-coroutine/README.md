...
#### Technical Deep Dive
*   **Cooperative vs. Preemptive:** Threads are typically **preemptively multitasked**. The OS scheduler can interrupt a thread at any time to give another thread a chance to run. Coroutines are **cooperatively multitasked**. A coroutine runs until it explicitly and voluntarily **yields** control.
...
*   **Stackless vs. Stackful:** Coroutines can be stackless (like in Kotlin, where they don't have a full OS-level stack) or stackful.
...
Ippudu manam process, thread, and coroutine ante ento, vaati madhya unna thedalu ento chusam. Next, manam oka Java thread yokka jeevitha chakram (lifecycle) gurinchi inka deep ga nerchukundam.