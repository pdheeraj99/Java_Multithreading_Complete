...
Oka thread create ayinappati nunchi, adi tana pani poorthi chesi chanipoye varaku, adi chala different states (dashalu) lo untundi. Ee states ni ardham chesukovadam, debugging and performance tuning lo chala critical. Oka manishi jeevitham lo childhood, adolescence, adulthood lanti stages unnattu, oka thread ki kuda oka jeevitha chakram (lifecycle) untundi.
...
Java specification (JLS) lo ee rendu sub-states ni kalipi `RUNNABLE` ane antaru. So, a thread that is waiting for the CPU is in the `RUNNABLE` state.
...
*   **OS (Operating System):** The actual scheduling—deciding which `RUNNABLE` thread gets to run on which **CPU core** and for how long—is the job of the OS scheduler. The JVM maps Java threads to OS threads, and then the OS takes over. Manam `Thread.setPriority()` lanti methods use chesi OS ki oka hint ivvachu, kani final decision OS de.

---

Ippudu manam thread yokka jeevitha chakram gurinchi nerchukunnam. Next, manam Java lo threads ni ela create cheyalo, a different approaches ento chuddam.